...
## Files
- `ai_ide/mcp_server.py` – stdio server exposing `initialize`, `tools/list`, `tools/call` and delegating to `execute_tool`.
- `ai_ide/mcp_servers.json` – config for MCP clients; points to the local stdio server.
- `ai_ide/mcp_health.py` – simple health check that starts the stdio server and runs `initialize` + `tools/list`.

## Run server
```bash
python ai_ide/mcp_server.py
```
The server reads one JSON object per line on stdin and writes one JSON object per line on stdout.

## Health check
```bash
python ai_ide/mcp_health.py
```
